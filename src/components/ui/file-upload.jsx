  ...
  (
    ...
    return (
      <div ref={ref} className={cn("w-full", className)} {...props}>
        ...
        {error && <p className="mt-2 text-sm text-destructive font-onest">{error}</p>}

        {files.length > 0 && (
          <div className="mt-4 space-y-2">
            {files.map((file, index) => {
              const FileIcon = getFileIcon(file)
              // Use file properties for stable key (name + size + lastModified)
              const fileKey = `${file.name}-${file.size}-${file.lastModified}`
              return (
                <div key={fileKey} className="flex items-center gap-3 p-3 bg-muted rounded-lg">
